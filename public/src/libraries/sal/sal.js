 ...
 * @param {HTMLElement} element
 */
const isAnimated = element => element.classList.contains(options.animateClassName)
 ...
 * @return {Boolean}
 */
const isDisabled = () => options.disabled || (typeof options.disabled === 'function' && options.disabled())
...
const getObservedElements = () => {
    const collection = [].filter.call(document.querySelectorAll(options.selector), element => !isAnimated(element, options.animateClassName))
...
const reset = (settings = {}) => {
    ...
    Array.from(document.querySelectorAll(options.selector)).forEach(clearAnimation)
